...
public class BookCreateFragment extends BaseFragment implements BookCreateView {
    ...
    TextView bookNameInput;

    @BindView(R.id.input_author)
    TextView bookAuthorInput;

    @BindView(R.id.input_position)
    TextView bookPositionInput;

    @BindView(R.id.input_description)
    TextView bookDescriptionInput;
    ...
    private void registerPublishButtonClickSubscription() {
        Disposable publishSubscription = RxView.clicks(publishButton)
    ...
    private void registerDescriptionInputSubscription() {
        Disposable descriptionSubscription = RxTextView.afterTextChangeEvents(bookDescriptionInput)
                ...
                .filter(new Predicate<TextViewAfterTextChangeEvent>() {
                    @Override
                    public boolean test(@NonNull TextViewAfterTextChangeEvent event) throws Exception {
                        return !event.view().getText().toString().contains("*#[]?");
                    }
                })
                .subscribe(new Consumer<TextViewAfterTextChangeEvent>() {
                    @Override
                    public void accept(@NonNull TextViewAfterTextChangeEvent event) throws Exception {
    ...
    private void registerCoverClickSubscription() {
        Disposable coverSubscription = RxView.clicks(cover)
                .switchMap(new Function<Object, ObservableSource<Response<BookCreateFragment, FileData>>>() {
                    @Override
                    public ObservableSource<Response<BookCreateFragment, FileData>> apply(@NonNull Object o) throws Exception {
    ...
    @Override
    public void OnReleased(final String newKey) {
    ...
    private void renderSticker(View sticker) {
        Bitmap b = Bitmap.createBitmap(sticker.getWidth(), sticker.getHeight(), Bitmap.Config.ARGB_8888);
        Canvas c = new Canvas(b);
        sticker.draw(c);
        MediaStore.Images.Media.insertImage(getContext().getContentResolver(), b, "Book sticker", "Glue it on the flyleaf");
    }
}
